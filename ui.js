  ...
  function _showUserActionSheet(user, allUsers) {
    ...
    const items = [
      ...
      !uIsAdmin
        ...
        : null,
      uIsAdmin && iAmSuper && !(user.isSuperAdmin && user.isSuperAdmin())
...
const UIExtended = {
  ...
  openEventDetail(event) {
    ...
    gallery.innerHTML = photos.map(url => `<img src="${url}" alt="" onclick="UI.openLightbox('${url}')">`).join('');

    const linksContainer = document.getElementById('event-detail-links');
    const links = event.links || [];
    linksContainer.innerHTML = links.map(l => `<a href="${UI._escHtml(l.url)}" target="_blank" rel="noopener">🔗 ${UI._escHtml(l.label || l.url)}</a>`).join('');
  ...
  _renderStory() {
    ...
    // Use featuredPhoto if set, otherwise first photo
    const photos = ev.photos || [];
    const photoUrl = ev.featuredPhoto || (photos.length ? photos[0] : '');
