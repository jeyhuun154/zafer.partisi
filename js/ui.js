...
const UI = (() => {
  ...
  function showScreen(screenId, direction = 'forward') {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        next.style.transition = 'opacity 0.35s cubic-bezier(0.25,0.46,0.45,0.94), transform 0.35s cubic-bezier(0.25,0.46,0.45,0.94)';
  ...
  function playSplash(onEnd) {
    ...
    if (!video) { setTimeout(onEnd, 2200); return; }

    // If video source exists and can play
    ...
    if (playPromise !== undefined) {
      playPromise
        .then(() => {
          ...
          // Safety: fire onEnd if video stalls > 10s
          setTimeout(() => {
            if (!video.ended) onEnd();
          }, 10000);
        })
        .catch(() => {
          // Video failed (not found / autoplay blocked) → show fallback
      ...
      video.addEventListener('ended', onEnd, { once: true });
      setTimeout(() => { if (!video.ended) onEnd(); }, 10000);
    }
  }
...
const UIExtended = {
  ...
  openSeeMoreModal(person) {
    ...
    if (photoWrap) {
      ...
      if (src) {
        photoWrap.innerHTML = `<img src="${src}" alt="${UI._escHtml(person.firstName)}">`;
      } else {
