...
const UI = (() => {
  ...
  function playSplash(onEnd) {
    ...
    function _exitSplash() {
      if (_exited) return;
      _exited = true;
      _currentScreen = ''; // clear so showScreen doesn't get confused
      if (splash) {
        ...
        requestAnimationFrame(() => {
          ...
          // Call onEnd AFTER splash is removed from DOM flow
    ...
    if (playPromise !== undefined) {
      playPromise
        .then(() => {
          clearTimeout(durationGuard); // trust the 'ended' event now
        })
        .catch(() => {
