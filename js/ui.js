...
const UI = (() => {
  ...
  function playSplash(onEnd) {
    ...
    function _exitSplash() {
      ...
      // Remove the inline <style> rule that has opacity:1!important on #screen-splash
      // Without this, splash.style.opacity='0' can never win against !important
      try {
        ...
        for (let i = 0; i < sheets.length; i++) {
          try {
            ...
            for (let j = rules.length - 1; j >= 0; j--) {
              if (rules[j].selectorText === '#screen-splash') {
      ...
      if (splash) {
        splash.style.cssText = 'opacity:0!important;pointer-events:none!important;z-index:-1!important;';
        requestAnimationFrame(() => {
          splash.classList.remove('screen--active');
          splash.style.cssText = 'display:none!important;';
          try { onEnd(); } catch (e) { console.error('[UI] playSplash onEnd error:', e); }
        });
      } else {
        try { onEnd(); } catch (e) { console.error('[UI] playSplash onEnd error:', e); }
      }
    ...
    // Hard timeout — no matter what, exit after 12 seconds
    const hardTimeout = setTimeout(_exitSplash, 12000);
    function _done() { clearTimeout(hardTimeout); _exitSplash(); }

    // No video element → show fallback then exit
    if (!video) {
      if (fallback) fallback.style.display = 'flex';
      setTimeout(_done, 2200);
      return;
    }

    // Check if video source is actually provided
    const source = video.querySelector('source');
    const hasSrc = (source && source.src && source.src !== window.location.href)
    ...
    // Bind ended before play to avoid race condition
    video.addEventListener('ended', _done, { once: true });

    // Max duration guard (10s)
    const durationGuard = setTimeout(_done, 10000);
...
const UIExtended = {
  ...
  openEventDetail(event) {
    ...
    if (photos.length === 0) {
      ...
      // Single image — keep it simple, no carousel chrome needed
      gallery.style.cssText = 'margin:0 0 16px;border-radius:14px;overflow:hidden;position:relative;';
      gallery.innerHTML = `
        <div style="position:relative">
          <img src="${UI._escHtml(photos[0])}" alt="" data-lightbox-url="${UI._escHtml(photos[0])}"
            style="width:100%;height:200px;object-fit:cover;display:block;cursor:pointer;background:var(--color-surface-elevated)">
          ${Auth.isAdmin() ? `<button class="event-photo-del-btn" data-idx="0" aria-label="Fotoğrafı sil"
