...
const People = (() => {
  ...
  function _render() {
    ...
    if (totalPages > 1) {
      ...
      if (paginEl) {
        ...
        // Always remove old listener before adding new one to prevent accumulation
        const listEl2 = document.getElementById('people-list');
        if (listEl2 && listEl2._paginScroll) {
  ...
  function _buildCard(person) {
    ...
    let photoHtml;
    if (person.photoURL) {
      photoHtml = `<div class="people-card__photo"><img src="${UI._escHtml(person.photoURL)}" alt="${UI._escHtml(person.firstName || '')}" loading="lazy" data-person-id="${person.id}"></div>`;
    } else {
  ...
  function _bindAdminActions(container) {
    ...
    // Handle broken images — replace with initials placeholder
    container.querySelectorAll('.people-card__photo img').forEach(img => {
      img.addEventListener('error', () => {
        const personId = img.dataset.personId;
        const person = _allPeople.find(p => p.id === personId);
        const initials = `${person?.firstName?.[0]||''}${person?.lastName?.[0]||''}`.toUpperCase();
        img.parentElement.outerHTML = `<div class="people-card__photo-placeholder">${initials}</div>`;
      });
    });
  }
