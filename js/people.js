...
const People = (() => {
  ...
  function _buildCard(person) {
    ...
    if (person.photoURL || person.photoBase64) {
      ...
      photoHtml = `<div class="people-card__photo"><img src="${src}" alt="${UI._escHtml(person.firstName)}" loading="lazy"></div>`;
    } else {
  ...
  async function savePerson(data, editId = null) {
    ...
    // Upload new photo to Firebase Storage if base64 provided
    ...
    await DB.savePerson({ ...person, photoBase64: photoURL ? '' : (data.photoBase64 || '') });
    return { success: true };
  }
