...
(async () => {
  ...
  async function _loadDaricaOverlay() {
    ...
    if (boundary.length >= 3) _applyDaricaOverlay(boundary);
  }
  ...
  function _applyDaricaOverlay(boundary) {
    ...
    // Create a custom pane below markers (markerPane=600, overlayPane=400)
    ...
    // Dim the world outside Darıca — markers at z=600 naturally appear above
    _daricaOverlay = L.polygon(
      [ [[-90,-180],[-90,180],[90,180],[90,-180]], boundary ],
      { stroke: false, fillColor: '#000', fillOpacity: 0.26,
        interactive: false, pane: 'dimPane' }
    ).addTo(_map);

    // Dashed border around Darıca
    _daricaBorder = L.polygon(boundary, {
      color: '#1B2E6E', weight: 2, opacity: 0.6,
      fillOpacity: 0, dashArray: '7 5', interactive: false, pane: 'dimPane'
    }).addTo(_map);
  }
  ...
  document.getElementById('save-event-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      // If triggered from map, also add a pin
  ...
  function _openEventModal(evData, ll = null) {
    ...
    else    { delete modal.dataset.eventLat; delete modal.dataset.eventLng; }
    ...
    items.forEach(item => {
      ...
      btn.onclick = async () => {
        ...
        if (item.action === 'detail') {
          ...
          const r = await Auth.setAdminStatus(user.id, true);
          ...
          if (!confirm(`${user.firstName} ${user.lastName} silinsin mi?`)) return;
