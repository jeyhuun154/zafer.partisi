...
(async () => {
  ...
  function _dpSimplify(points, tolerance) {
    if (points.length <= 2) return points;
    // Find the point with the maximum distance from the line start→end
    const [x1, y1] = [points[0][0], points[0][1]];
    const [x2, y2] = [points[points.length - 1][0], points[points.length - 1][1]];
    ...
    if (maxDist > tolerance) {
      const left  = _dpSimplify(points.slice(0, maxIdx + 1), tolerance);
      const right = _dpSimplify(points.slice(maxIdx), tolerance);
  ...
  async function _finishDraw() {
    ...
    // Simplify to reduce Firestore payload — tolerance ~0.0002 deg ≈ 20m
    let simplified = _dpSimplify(_drawPoints, 0.0002);
    // Ensure at minimum 3 and cap at 400 points to stay well within Firestore 1MB doc limit
    if (simplified.length < 3) simplified = _drawPoints.slice(0, Math.min(_drawPoints.length, 400));
    if (simplified.length > 400) {
      // Thin further by stepping
      const step = Math.ceil(simplified.length / 400);
