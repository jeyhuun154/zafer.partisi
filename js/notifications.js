...
const Notifications = (() => {
  ...
  async function init(userId) {
    ...
    // FCM token (best-effort, non-blocking)
    FirebaseService.getFCMToken()
      .then(token => { if (token) FirebaseService.saveFCMToken(userId, token); })
      .catch(() => {});
  ...
  function _renderPanel() {
    ...
    if (isSuperAdmin && _pendingActionItems.length > 0) {
      ...
      _pendingActionItems.forEach(action => {
        ...
        if (action.type === 'delete') {
          typeLabel = `<strong>${_esc(action.targetUserName)}</strong> silinmek istiyor`;
          icon = '🗑️';
    ...
    if (_inboxItems.length > 0) {
      ...
      _inboxItems.forEach(notif => {
        const isNew = notif.createdAt && (_lastSeen > 0) ? (notif.createdAt > (_lastSeen - 5000)) : false;
        html += `
