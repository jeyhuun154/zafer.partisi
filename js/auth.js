...
const Auth = (() => {
  ...
  async function login(firstName, lastName, code) {
    ...
    // ── Check for any pending actions (covers deletion & promotion) ──
    const pending = await _checkPendingAction(matched.id);
    if (pending && pending.type === 'delete') {
      return {
        success: false,
        error: `Hesabınız ${pending.requesterName} tarafından silinmek üzere işaretlendi. Üst yetkili onayı bekleniyor.`
      };
    }
