...
const FirebaseService = (() => {
  ...
  async function init() {
    ...
    // enablePersistence with synchronizeTabs enables multi-tab IndexedDB caching.
    ...
    // Any other error code is logged as a warning only — the app still works without it.
    try {
      await _db.enablePersistence({ synchronizeTabs: true });
    } catch (e) {
      const code = e?.code || '';
      if (code !== 'failed-precondition' && code !== 'unimplemented') {
        console.warn('[Firebase] Persistence:', code || e);
      }
    }
