...
const FirebaseService = (() => {
  ...
  async function init() {
    ...
    // enablePersistence enables offline IndexedDB caching across tabs.
    // These two error codes are expected and completely harmless — ignore silently:
    //   failed-precondition → another tab already owns persistence (works fine in that tab)
    //   unimplemented        → browser doesn't support IndexedDB (app still works online)
    // Everything else is also swallowed — persistence is a nice-to-have, never fatal.
